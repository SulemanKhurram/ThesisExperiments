...
if (args.dataset == 'cifar10'):
    ...
    utils.writeLogs_KCV("| Preparing CIFAR-10 dataset...")
    ...
    utils.writeLogs_KCV("| Preparing CIFAR-100 dataset...")
    ...
    utils.writeLogs_KCV("| Preparing MNIST dataset...")
    ...
    utils.writeLogs_KCV("| Preparing FASHIONMNIST dataset...")
    ...
    utils.writeLogs_KCV("| Preparing STL10 dataset...")
    ...
    utils.writeLogs_KCV("| Preparing Origa dataset...")
    ...
    trainset = OrigaDataset('./test.txt',origa_transform_train )
    testset = OrigaDataset('./test.txt',origa_transform_test )
    allset = OrigaDataset('./origaAll.txt', origa_transform_test)
...
if args.resume:
    ...
    utils.writeLogs_KCV('| Resuming from checkpoint...')
    ...
    print("test0.1")
    utils.writeLogs_KCV('| Building net type [' + args.net_type + ']...')
    print("test0.1")
...
def train(epoch):
    ...
    for batch_idx, (inputs_value, targets) in enumerate(trainloader):
        ...
        sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f Acc@1: %.3f%%'
                %(epoch, cf.num_epochs, batch_idx+1,
                    (len(trainset)//cf.batch_size)+1, loss.item(), (100*correct/total)))
        utils.writeLogs_KCV(str('| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f Acc@1: %.3f%%'
                %(epoch, cf.num_epochs, batch_idx+1,
    ...
    utils.writeLogs_KCV(str(diagnostics_to_write))
    with open(logfile, 'a') as lf:
        lf.write(str(diagnostics_to_write))


def printMetrics(testTargets,testPredicts, epoch,kCount):

    print(epoch + " f1_score:" + str(f1_score(testTargets, testPredicts, average="macro")))
    utils.writeLogs_KCV(epoch + " f1_score:" + str(f1_score(testTargets, testPredicts, average="macro")))
    print(epoch + " overall precision:" + str(precision_score(testTargets, testPredicts, average="macro")))
    utils.writeLogs_KCV(epoch + " overall precision:" + str(precision_score(testTargets, testPredicts, average="macro")))
    print(epoch + " overall recall : " + str(recall_score(testTargets, testPredicts, average="macro")))
    utils.writeLogs_KCV(epoch + " overall recall : " + str(recall_score(testTargets, testPredicts, average="macro")))
    ...
    utils.writeLogs_KCV(epoch + " False positive : " + str(fpr))
    print (epoch + " True positive" + str(tpr))
    utils.writeLogs_KCV(epoch + " True positive : " + str(tpr))
    print(epoch + " Thresholds : " + str(thresholds))
    utils.writeLogs_KCV(epoch + " Thresholds : " + str(thresholds))

    print(epoch + " ROC_AUC : " + str(roc_auc))
    utils.writeLogs_KCV(epoch + " ROC_AUC : " + str(roc_auc))
    confuse = confusion_matrix(testTargets, testPredicts)
    print(epoch + " Confusion Matrix : " + str(confuse))
    utils.writeLogs_KCV(epoch + " Confusion Matrix : " + str(confuse))
    ...
    utils.writeLogs_KCV(epoch + str(classification_report(testTargets, testPredicts, target_names=target_names, digits=4)))
    if epoch == "Final":
        #ROC curve
        plt.figure()
        lw = 2
        ...
        plt.savefig('./plots/' + 'roc_k'+str(kCount)+'.png', format='png', dpi=300)
        plt.show()
        plt.close()
        ...
        plt.savefig('./plots/' + 'heatmap_k'+str(kCount)+'.png', format='png', dpi=300)
        plt.show()
        plt.close()
        ...
        plt.savefig('./plots/' + 'trainLoss_k'+str(kCount)+'.png', format='png', dpi=300)
        plt.show()
        plt.close()


def test(epoch, kCount):
    global best_acc
    net.eval()
    testTargets = []
    testPredicts = []
    ...
    printMetrics(testTargets, testPredicts, "Epoch " + str(epoch), kCount)
    # Save checkpoint when best model
    acc =(100*correct.item()/total)
    print("\n| Validation Epoch #%d\t\t\tLoss: %.4f Acc@1: %.2f%%" %(epoch, loss.item(), acc))
    utils.writeLogs_KCV(str("\n| Validation Epoch #%d\t\t\tLoss: %.4f Acc@1: %.2f%%" %(epoch, loss.item(), acc)))
    ...
    if acc > best_acc:
        ...
        utils.writeLogs_KCV(str('| Saving Best model...\t\t\tTop1 = %.2f%%' %(acc)))
        state = {
    ...
    if epoch == cf.num_epochs:
        printMetrics(testTargets, testPredicts, "Final",kCount)


def kFoldCV():
    global trainloader, testloader, testset, trainset,trainLoss
    cv = model_selection.StratifiedKFold(n_splits=k, shuffle=True, random_state=None)
    kCount = 1;
    for train_indices, test_indices in cv.split(allset,allset.labels):
        trainLoss = []
        print('For k = ' + str(kCount))
        utils.writeLogs_KCV('For k = ' + str(kCount))
        trainset = SubsetRandomSampler(train_indices)
        testset = SubsetRandomSampler(test_indices)
        trainloader = torch.utils.data.DataLoader(allset, batch_size=cf.batch_size, shuffle=False, num_workers=4, sampler=trainset)
        testloader = torch.utils.data.DataLoader(allset, batch_size=cf.batch_size, shuffle=False, num_workers=4, sampler=testset)
...
utils.writeLogs_KCV('| Training Epochs = ' + str(cf.num_epochs))
print('| Initial Learning Rate = ' + str(cf.lr))
utils.writeLogs_KCV('| Initial Learning Rate = ' + str(cf.lr))
print('| Optimizer = ' + str(cf.optim_type))
utils.writeLogs_KCV('| Optimizer = ' + str(cf.optim_type))
...
if args.dataset == 'origa':
    ...
    utils.writeLogs_KCV(str('| Elapsed time : %d:%02d:%02d'  %(cf.get_hms(elapsed_time))))
print('\n[Phase 4] : Testing model')
utils.writeLogs_KCV('\n[Phase 4] : Testing model')
print('* Test results : Acc@1 = %.2f%%' %(best_acc))
utils.writeLogs_KCV(str('* Test results : Acc@1 = %.2f%%' %(best_acc)))
